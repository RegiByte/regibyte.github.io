...
const ColorBlock: React.FC<ColorBlockProps> = ({ color, onRemove }) => {
  return (
    <div
      ...
      <XMarkIcon className="absolute top-0 right-0 hidden text-xs group-hover:inline-flex">
        X
      </XMarkIcon>
    </div>
  );
...
const NewColorPicker: React.FC<NewColorPickerProps> = ({
  initialColor = '#000000',
  onAdd,
}) => {
  ...
  return (
    <div className="relative inline-flex items-center justify-center md:justify-start gap-2 w-full md:w-auto">
      <button
        ...
        className={'rounded bg-slate-300 px-2 py-1 text-neutral-700'}>
